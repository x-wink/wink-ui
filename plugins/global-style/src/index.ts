...
import fs from 'fs';
import type { Plugin, HtmlTagDescriptor, HmrContext } from 'vite';
...
function searchGlobalStyle(rootDir: string, options: PluginOptions, data: StyleData): Array<string> {
...
export default (options: Partial<PluginOptions> = {}): Plugin => {
    ...
    function scan() {
        if (!scanned) {
            if (!assetsPath) {
                assetsPath = path.resolve(process.cwd(), opts.sourcePath);
            ...
            GLOBAL_STYLES_DATA.forEach((data) => {
                ...
                opts.debug &&
                    console.info(`[global style] ${data.name} search result: \n${data.globalStylePaths.join('\n')}`);
            });
    ...
    return {
        name: 'wink:global-style',
        enforce: 'pre',
        transformIndexHtml: {
            enforce: 'pre',
            transform(_, { filename }: { filename: string }): Array<HtmlTagDescriptor> {
                ...
                if (opts.cssEnabled) {
                    ...
                    CSSFilePaths.forEach((filePath) => {
                        filePath = filePath.replace(assetsPath, '/' + opts.sourcePath).replace(/\\(\\)?/g, '/');
        ...
        transform(code: string, id: string) {
            ...
            GLOBAL_STYLES_DATA.forEach((data) => {
                ...
                if (data.extensionRegex.test(id) && data.globalStylePaths.length > 0 && data.isEnabled(opts)) {
                    // inject the global styles to corresponding files
                    const globalImport = data.globalStylePaths
                        .map((filePath) => {
                            filePath = path.relative(path.join(id, '..'), filePath).replace(/\\(\\)?/g, '/');
